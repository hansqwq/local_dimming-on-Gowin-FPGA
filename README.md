...
* **Mini-LED驱动 (Mini-LED Driving)**: 通过SPI接口控制LED驱动芯片（SPI7001），实现对360颗LED灯珠的精确亮度调节。
...
* `lvds_7to1_rx_top.v`: LVDS 7:1接收器顶层模块。负责接收外部输入的LVDS差分信号，通过内嵌的`bit_align_ctl.v`和`word_align_ctl.v`进行位同步和字同步，并利用`LVDS71RX_1CLK8DATA`原语进行解串，最终输出并行的RGB视频数据及同步信号。
    * `bit_align_ctl.v`: LVDS位对齐控制模块，用于动态调整采样时钟相位以正确接收串行数据。
    * `word_align_ctl.v`: LVDS字对齐控制模块，确保正确地将串行比特流组合成有效的数据字。
* `led_backlight.v` : 分区背光亮度计算核心模块。接收并行RGB视频数据和同步信号，将屏幕划分为24x15个区域，计算每个区域的平均灰度值作为该区域LED的亮度值，并将结果输出。
* `fifo_top.v` : FIFO缓冲模块，用于在`led_backlight.v`计算得到的亮度数据和后续的SRAM写入/LED驱动模块之间进行数据缓冲和时钟域转换。
...
* `sram_top_gowin_top.v` : SRAM顶层控制模块，用于存储各个分区的亮度数据。
* `SPI7001_gowin_top.v` : 针对特定LED驱动芯片（SPI7001）的SPI接口驱动模块，将SRAM中存储的亮度数据通过SPI协议发送给LED背光板。
...
* `lvds_7to1_rx_defines.v` / `lvds_7to1_tx_defines.v` : Verilog定义文件，用于配置LVDS收发器的各种参数，如接口标准、颜色深度、通道数等。
